...
9 月 9 日已在生产创建日期索引，旧首页同一请求实测从约 45 秒降至 7.630 秒（HTTP 200，531562 字节）。用户已接受该耗时；下文低于 5 秒保留为优化目标，不作为合并门槛。分页代码尚未部署，不能把索引收益当作分页验收结果。

本次部署先只读检查已有索引，不重复执行建索引操作。代码更新后需要在宝塔重启 Web，使路由变更生效，再检查主题切换、分页、筛选和统计。配置版本及提示词应保持原值，模型切换功能一并保留。

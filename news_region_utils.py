...
def _call_llm(system_prompt, user_prompt, max_retries=3):
    ...
    for attempt in range(max_retries):
        client = _pool.get_client(*model_credentials("region"))
        try:
            resp = client.chat.completions.create(
                **model_arguments("region"),
                messages=[
...
def call_region_and_business_type_llm(title, content, business_type_catalog=None, aliases=None, max_retries=3):
    ...
    user_prompt = value("NEWS_ITEM_SHORT_CONTENT_USER_PROMPT_GJJ_REGION").format(
        title=(title or "").strip(),
        content=(content or "").strip(),
        business_type_catalog=format_business_type_catalog(business_type_catalog or {}),
    )
    raw_text = _call_llm(
        value("NEWS_ITEM_SHORT_CONTENT_SYSTEM_PROMPT_GJJ_REGION"),
        user_prompt,
        max_retries=max_retries,
    )
    payload = _extract_json_payload(raw_text)
...
def call_business_type_llm(title, content, business_type_catalog=None, aliases=None, max_retries=3):
    user_prompt = value("NEWS_BUSINESS_TYPE_USER_PROMPT_GJJ").format(
        title=(title or "").strip(),
        content=(content or "").strip(),
        business_type_catalog=format_business_type_catalog(business_type_catalog or {}),
    )
    raw_text = _call_llm(
        value("NEWS_BUSINESS_TYPE_SYSTEM_PROMPT_GJJ"),
        user_prompt,
        max_retries=max_retries,
    )
    payload = _extract_json_payload(raw_text)
...
def call_region_llm(title, content, max_retries=3):
    user_prompt = value("NEWS_REGION_USER_PROMPT_GJJ").format(
        title=(title or "").strip(),
        content=(content or "").strip(),
    )
    raw_text = _call_llm(
        value("NEWS_REGION_SYSTEM_PROMPT_GJJ"),
        user_prompt,
        max_retries=max_retries,
    )
